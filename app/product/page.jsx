...
import { get, like ,add} from '@/service/product.service';
  ...
  const toggleFavorite = async (index, id) => {
    try {
      setFavorites((prevFavorites) => {
        const isFavorite = !!prevFavorites[index];
        const newFavorites = {
          ...prevFavorites,
          [index]: !isFavorite,
        };
          ...
          <div className="flex items-start justify-between">
            <div className="left relative sm:hidden bottom-[7px]">
              <div className="flex relative top-[7px] z-30 flex-col gap-[16px] price-regular pt-[19px] pb-[38px] px-[18px] w-[292px] h-[522px] bg-[#fff] rounded-[8px]">
                ...
                <div className='h-[82px] w-[256px] bg-[#F2F2F2] rounded-[8px]'></div>
            ...
            <div className='right'>
              ...
              <div className="flex w-[924px] sm:w-[350px] flex-wrap sm:justify-center items-center justify-between gap-x-[24px] gap-y-[40px] mb-[111px] sm:mb-[70px]">
                {products.map((item, index) => (
                  <div key={index} className="card bg-[#Fff] rounded-[5px] w-[292px] h-[416px] pt-[25px] flex flex-col">
                    <i
                    ...
                    ></i>
                    <Image onClick={()=>handleNavigate(item.product_id)} className="relative cursor-pointer hover:scale-[1.04] transition-all duration-200 left-[20px] h-[205px] w-[240px] bottom-[10px]" width={292} height={92} src={item.image_url[1]} alt="products image" />

                    <div onClick={()=>handleNavigate(item.product_id)} className="title p-[20px] w-[230px] flex flex-col gap-[14px] pb-[14px]">
                      <h2 className="text-[20px] font-normal">{item.product_name}</h2>
